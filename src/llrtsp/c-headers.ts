import * as constants from './constants';
import { enumToMap, IEnumMap } from './utils';

type Encoding = 'none' | 'hex';

export class CHeaders {
    public build(): string {
        let res = '';

        res += '#ifndef LLLLRTSP_C_HEADERS_\n';
        res += '#define LLLLRTSP_C_HEADERS_\n';

        res += '#ifdef __cplusplus\n';
        res += 'extern "C" {\n';
        res += '#endif\n';

        res += '\n';

        const errorMap = enumToMap(constants.ERROR);
        const methodMap = enumToMap(constants.METHODS);

        res += this.buildEnum('llrtsp_errno', 'HPE', errorMap);
        res += '\n';
        res += this.buildEnum('llrtsp_flags', 'F', enumToMap(constants.FLAGS),
            'hex');
        res += '\n';
        res += this.buildEnum('llrtsp_type', 'RTSP',
            enumToMap(constants.TYPE));
        res += '\n';
        res += this.buildEnum('llrtsp_finish', 'RTSP_FINISH',
            enumToMap(constants.FINISH));
        res += '\n';
        res += this.buildEnum('llrtsp_method', 'RTSP', methodMap);

        res += '\n';

        res += this.buildMap('RTSP_ERRNO', errorMap);
        res += '\n';
        res += this.buildMap('RTSP_METHOD', methodMap);
        res += '\n';

        res += '\n';

        res += '#ifdef __cplusplus\n';
        res += '}  /* extern "C" */\n';
        res += '#endif\n';
        res += '#endif  /* LLLLRTSP_C_HEADERS_ */\n';

        return res;
    }

    private buildEnum(name: string, prefix: string, map: IEnumMap,
                      encoding: Encoding = 'none'): string {
        let res = '';

        res += `enum ${name} {\n`;
        const keys = Object.keys(map);
        keys.forEach((key, i) => {
            const isLast = i === keys.length - 1;

            let value: number | string = map[key];

            if (encoding === 'hex') {
                value = `0x${value.toString(16)}`;
            }

            res += `  ${prefix}_${key.replace(/-/g, '')} = ${value}`;
            if (!isLast) {
                res += ',\n';
            }
        });
        res += '\n};\n';
        res += `typedef enum ${name} ${name}_t;\n`;

        return res;
    }

    private buildMap(name: string, map: IEnumMap): string {
        let res = '';

        res += `#define ${name}_MAP(XX) \\\n`;
        Object.keys(map).forEach((key) => {
            res += `  XX(${map[key]!}, ${key.replace(/-/g, '')}, ${key}) \\\n`;
        });
        res += '\n';

        return res;
    }
}
