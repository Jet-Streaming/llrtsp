...
export class RTSP {
  ...
  constructor(private readonly llparse: LLParse,
              private readonly mode: RTSPMode = 'loose') {
    const p = llparse;
    ...
    this.span = {
      body: p.span(p.code.span('llrtsp__on_body')),
      headerField: p.span(p.code.span('llrtsp__on_header_field')),
      headerValue: p.span(p.code.span('llrtsp__on_header_value')),
      status: p.span(p.code.span('llrtsp__on_status')),
    };

    /* tslint:disable:object-literal-sort-keys */
    this.callback = {
      // User callbacks
      onHeadersComplete: p.code.match('llrtsp__on_headers_complete'),
      onMessageBegin: p.code.match('llrtsp__on_message_begin'),
      onMessageComplete: p.code.match('llrtsp__on_message_complete'),
      onChunkHeader: p.code.match('llrtsp__on_chunk_header'),
      onChunkComplete: p.code.match('llrtsp__on_chunk_complete'),

      // Internal callbacks `src/rtsp.c`
      beforeHeadersComplete:
        p.code.match('llrtsp__before_headers_complete'),
      afterHeadersComplete: p.code.match('llrtsp__after_headers_complete'),
      afterMessageComplete: p.code.match('llrtsp__after_message_complete'),
    };
  ...
  public build(): IRTSPResult {
    const p = this.llparse;

    p.property('i64', 'content_length');
    p.property('i8', 'type');
    p.property('i8', 'method');
    p.property('i8', 'rtsp_major');
    p.property('i8', 'rtsp_minor');
  ...
  private buildLine(): void {
    ...
    n('req_or_res_method')
      ...
      .match('RTSP/', this.update('type', TYPE.RESPONSE, 'res_rtsp_major'))
      .otherwise(p.error(ERROR.INVALID_CONSTANT, 'Invalid word encountered'));

    // Response

    n('start_res')
      .match('RTSP/', n('res_rtsp_major'))
      .otherwise(p.error(ERROR.INVALID_CONSTANT, 'Expected RTSP/'));

    n('res_rtsp_major')
      .select(MAJOR, this.store('rtsp_major', 'res_rtsp_dot'))
      .otherwise(p.error(ERROR.INVALID_VERSION, 'Invalid major version'));

    n('res_rtsp_dot')
      .match('.', n('res_rtsp_minor'))
      .otherwise(p.error(ERROR.INVALID_VERSION, 'Expected dot'));

    n('res_rtsp_minor')
      .select(MINOR, this.store('rtsp_minor', 'res_rtsp_end'))
    ...
    n('req_spaces_before_url')
      .match(' ', n('req_spaces_before_url'))
      .otherwise(url.entry.normal);

    url.exit.toRTSP
      .otherwise(n('req_rtsp_start'));

    // url.exit.toRTSP09
    //   .otherwise(
    //     this.update('rtsp_major', 0,
    //       this.update('rtsp_minor', 9, 'header_field_start')),
    //   );

    // const isSource = this.isEqual('method', METHODS.SOURCE, {
    //   equal: n('req_rtsp_major'),
    //   notEqual: p.error(ERROR.INVALID_CONSTANT,
    //     'Expected SOURCE method for ICE/x.x request'),
    // });

    n('req_rtsp_start')
      .match('RTSP/', n('req_rtsp_major'))
      // .match('ICE/', isSource)
      .match(' ', n('req_rtsp_start'))
      .otherwise(p.error(ERROR.INVALID_CONSTANT, 'Expected RTSP/'));

    n('req_rtsp_major')
      .select(MAJOR, this.store('rtsp_major', 'req_rtsp_dot'))
      .otherwise(p.error(ERROR.INVALID_VERSION, 'Invalid major version'));

    n('req_rtsp_dot')
      .match('.', n('req_rtsp_minor'))
      .otherwise(p.error(ERROR.INVALID_VERSION, 'Expected dot'));

    n('req_rtsp_minor')
      .select(MINOR, this.store('rtsp_minor', 'req_rtsp_end'))
      .otherwise(p.error(ERROR.INVALID_VERSION, 'Invalid minor version'));
  ...
  private buildHeaderValue(): void {
    ...
    const afterHeadersComplete = p.invoke(callback.afterHeadersComplete, {
      ...
      // non-chunked `Transfer-Encoding` for request, see `src/native/rtsp.c`
