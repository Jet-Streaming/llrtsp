...
export class URL {
  ...
  constructor(private readonly llparse: LLParse,
    ...
    if (separateSpans) {
      table.set('schema', p.span(p.code.span('llrtsp__on_url_schema')));
      table.set('host', p.span(p.code.span('llrtsp__on_url_host')));
      table.set('path', p.span(p.code.span('llrtsp__on_url_path')));
      table.set('query', p.span(p.code.span('llrtsp__on_url_query')));
      table.set('fragment',
        p.span(p.code.span('llrtsp__on_url_fragment')));
    } else {
      table.set('url', p.span(p.code.span('llrtsp__on_url')));
    }
  }
  ...
  public build(): IURLResult {
    ...
    // Adaptors
    const toRTSP = this.node('to_rtsp');
    // const toHTTP09 = this.node('to_rtsp_09');

    const skipToRTSP = this.node('skip_to_rtsp')
      .skipTo(toRTSP);

    // const skipToHTTP09 = this.node('skip_to_rtsp09')
    //   .skipTo(toHTTP09);

    // const skipCRLF = this.node('skip_lf_to_rtsp09')
    //   .match('\r\n', toRTSP09)
